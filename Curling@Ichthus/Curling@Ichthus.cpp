...
struct context {
    ...
    std::vector<int> best_tails_counter;		// number of generators for the longest tail lengths (per worker)
};
...
void backtracking_step(context& ctx) {
    ...
    while ((ctx.c_cand * ctx.p_cand) > ctx.seq.size()) {                    // trick 1: we look for a pair for which the product is not too large;
        ...
        if (ctx.c_cand <= ctx.seq.back()) {                                 // trick 2 
            ...
            int tail_length = ctx.seq.back() + ctx.length;
            auto ind = std::find(ctx.seq_map[tail_length].begin(), ctx.seq_map[tail_length].end(), ctx.seq.size() - 1);
            ctx.seq_map[tail_length].erase(ind);                                  // remove entry from reverse map
...
void append(context& ctx) {
    ...
    if (ctx.best_tails[len] < (int16_t)tail) {                  // and update maximum values for this thread
        ...
        ctx.best_tails_counter[len] = 1;
    }
    else if (ctx.best_tails[len] == (int16_t)tail)
        ctx.best_tails_counter[len]++;
}
...
// parse tails to JSON format
std::string parseToJson(context& ctx) {
    std::string json = "{\"lines\":[";
    int record = 0;
    for (int i = 0; i <= ctx.length; i++) {
        if (ctx.best_tails[i] > record) {
            record = ctx.best_tails[i];
            std::string tail = "\"[";
            for (int j : ctx.best_grts[i]) {
                if (j == 0)
                    break;
            ...
            tail.pop_back();
            tail += "]\"";
            json += "{\"genlen\":" + std::to_string(i) + ",\"taillen\":" + std::to_string(ctx.best_tails[i]) + ",\"freq\":" + std::to_string(ctx.best_tails_counter[i]) + ",\"generator\":" + tail + "},";
        }
    }
    json.pop_back();
    json += "]}";
...
void worker(int thread_number) {
    ...
    while (true) {
        ...
        auto res = cli.Get("/cs_get.php");  // request a job from the server

        char date[26] = {};
        time_t now = time(NULL);
        UPDATE_TIME;

        // check if the response contains an HTML character or was 'empty' (indicates failure)
        if (res->body.find('<') < res->body.length() || res->body.length() <= 4) {
            if (!tries) {
                active_threads--;   // if this was the first fail, decrease active thread counter
            }
            tries++;
            if (tries > 10) {       // after 10 tries, just give up
                fprintf(OUTPUT, "[%.15s] Thread %02d: bad response, timed out and quitting\n", &date[4], thread_number);
            }
            if (!active_threads) {  // if there are no more active threads, quit to terminate program
                fprintf(OUTPUT, "[%.15s] Thread %02d: no more active threads, quitting\n", &date[4], thread_number);
                break;
            }
            fprintf(OUTPUT, "[%.15s] Thread %02d: bad response, retrying in %d seconds\n", &date[4], thread_number, (int)pow(tries, 5));
            fprintf(OUTPUT, "[%.15s] Active threads: %d/%d\n", &date[4], active_threads, thread_count);

            // incremental sleep: tries^5 (total sleep time approx. a full day)
            std::this_thread::sleep_for(seconds((int)pow(tries, 5)));
            continue;
        }
        if (tries) {    // if there were any retries, reset and increment active thread counter
            active_threads++;
            tries = 0;
        }

        // parse ID, length and number of lines from the job
        std::istringstream stringReader{ res->body };
        int ID;
        stringReader >> ID;
        int len;
        stringReader >> len;
        int num_lines;
        stringReader >> num_lines;

        // calculate maximum depth for this length
        int max_depth = largestPowerOf2(len);

        now = time(NULL);
        UPDATE_TIME;
        fprintf(OUTPUT, "[%.15s] Thread %02d started! (%d, %d, %d)\n", &date[4], thread_number, len, num_lines, ID);
        ...
        */
        ...
        ctx.best_tails_counter.resize(len + 1);   // adjust for +1 index to simplify accessing elements

        v16_t cmb;
        std::queue<v16_t> cmbs;			    // queue with the combinations to process by workers

        // parse remaining part of the job into the first combination
        int number;
        while (stringReader >> number)
            cmb.push_back(number);

        uint16_t depth = cmb[0];

        // fill up the queue with all combinations until the first curl (depth 1) exceeds the sequence length (invalid)
        while (cmbs.size() < num_lines) {
            if (cmb[1] > ctx.length) {
                /*
        ...
        int tries = 0;
        std::string json = parseToJson(ctx);
        std::string body = "id=" + std::to_string(ID) + "&json=" + json;
        res = cli.Post("/cs_post.php", body, "application/x-www-form-urlencoded");
        while (res->body != "true") {
            std::cout << res->body << std::endl;
            tries++;
            if (tries > 10) {
                fprintf(OUTPUT, "[%.15s] Thread %02d timed out posting results, continuing\n", &date[4], thread_number);
            }
            res = cli.Post("/cs_post.php", body, "application/x-www-form-urlencoded");
            now = time(NULL);
            UPDATE_TIME;
            fprintf(OUTPUT, "[%.15s] Thread %02d failed to post results, retrying in 1 second\n", &date[4], thread_number);
...
int main(int argc, char** argv) {
     /*if (argc != 2) {
         std::cout << "Need exactly one CL argument: number of threads" << std::endl;
         return 0;
     }*/

    thread_count = std::thread::hardware_concurrency(); //atoi(argv[1]);
    active_threads = thread_count;

    char date[26] = {};
    time_t now = time(NULL);
    UPDATE_TIME;
    fprintf(OUTPUT, "[%.15s] Started with total thread count %i\n", &date[4], thread_count);
    ...
    for (int thread_number = 1; thread_number <= thread_count; ++thread_number)
        thread_vector.emplace_back(std::thread(worker, thread_number));
